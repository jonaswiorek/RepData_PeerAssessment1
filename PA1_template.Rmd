...
activityNoNa <- mutate(group_by(activity, interval), intervalSteps = median(steps, na.rm = TRUE))
activityNoNa <- mutate(activityNoNa, stepsNoNa = ifelse(is.na(steps), intervalSteps, steps))
dailySteps <- summarize(group_by(activityNoNa, date), steps = sum(stepsNoNa, na.rm = TRUE))
hist(dailySteps$steps, breaks=20)
mean(dailySteps$steps)
median(dailySteps$steps)
```
